...
- We want to summarize empirical patterns in the data.
- We want to estimate common reduced-form specifications in Julia.
:::

::: {.callout-important title="Learning Goals" appearance="simple"}
- Produce descriptive statistics and correlations.
- Estimate OLS, instrumental-variable, and fixed-effects models.
- Implement difference-in-differences and regression-discontinuity analyses.
:::

## Generate the Panel Data

- This synthetic example is inspired by the minimum-wage application in [Callaway and Sant'Anna (2021)](https://doi.org/10.1016/j.jeconom.2020.12.001).
- It is not a replication of their data or empirical results.
...
- The treatment effect is known, so we can compare it with the estimates.
...
julia_values = [1, 2, 3, 4]
@rput julia_values

r_mean = rcopy(R"mean(julia_values)")
```
